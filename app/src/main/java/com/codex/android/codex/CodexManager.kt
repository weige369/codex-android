    ...
    suspend fun downloadWithProgress(
        ...
        try {
            ...
            if (responseCode != HttpURLConnection.HTTP_OK) {
                Log.w(TAG, "下载返回 $responseCode，尝试备用地址...")
                return@withContext downloadFromFallback(onProgressCallback)
            }

            val contentLength = connection.contentLength.toLong()
            Log.i(TAG, "开始下载 Codex CLI (大小: ${contentLength / 1024 / 1024}MB)")
            ...
            // 尝试备用地址
            try {
                downloadFromFallback(onProgressCallback)
            } catch (e2: Exception) {
                Log.e(TAG, "备用下载也失败", e2)
                false
            }
        }
    }
