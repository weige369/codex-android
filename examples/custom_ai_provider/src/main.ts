...
type ExtendedProviderConfig = ToolPkg.AiProviderConfig & {
  timeout?: number | string;
};

type CompatibleHttpResponse = OkHttpResponse & {
  ok?: boolean;
  success?: boolean;
  code?: number;
  status?: number;
...
function requireConfigField(
  config: ToolPkg.AiProviderConfig,
  fieldName: keyof ToolPkg.AiProviderConfig,
  displayName: string
): string {
...
function buildHeaders(config: ToolPkg.AiProviderConfig): Record<string, string> {
  const headers: Record<string, string> = {
    "Content-Type": "application/json"
  };
  const customHeaders =
    config && typeof config.customHeaders === "object" ? config.customHeaders : {};
...
function buildModelsEndpoint(apiEndpoint: string): string {
...
function mapPromptTurnsToMessages(chatHistory?: ToolPkg.PromptTurn[]): ProviderMessage[] {
...
async function parseJsonSafely(response: CompatibleHttpResponse): Promise<OpenAiCompatibleResponse> {
  if (!response) {
...
function ensureHttpOk(response: CompatibleHttpResponse, payload: unknown): void {
  const ok = response && (
    (typeof response.isSuccessful === "function" && response.isSuccessful()) ||
    response.ok === true ||
  ...
  throw new Error(
    `HTTP 请求失败${statusCode ? ` (${statusCode})` : ""}${statusText ? `: ${statusText}` : ""}${bodyText ? ` ${bodyText}` : ""}`
  );
}

function createHttpClient(timeoutMs: number): OkHttpClient {
...
async function httpRequest(options: HttpRequestOptions): Promise<CompatibleHttpResponse> {
...
function extractUsage(payload: OpenAiCompatibleResponse): { input: number; cachedInput: number; output: number } {
...
function emitChunks(text: string, usage: { input: number; cachedInput: number; output: number }): void {
...
function mergeUsage(
  current: { input: number; cachedInput: number; output: number },
  nextUsage?: OpenAiCompatibleUsage
...
function processSsePayload(state: StreamState, payloadText: string): void {
  if (!payloadText || payloadText === "[DONE]") {
    return;
  }
  const payload = JSON.parse(payloadText) as OpenAiCompatibleResponse & {
    choices?: Array<{
      delta?: {
        content?: string;
      };
  ...
  const deltaText =
    normalizeText(payload.choices?.[0]?.delta?.content) ||
...
function processStreamChunk(state: StreamState, event: HttpStreamEvent): void {
...
export async function listModels(event: ToolPkg.AiProviderListModelsEvent) {
...
export async function sendMessage(event: ToolPkg.AiProviderSendMessageEvent) {
  const payload = event.eventPayload;
  const config = payload.config as ExtendedProviderConfig;
  const apiEndpoint = requireConfigField(config, "apiEndpoint", "API Endpoint");
...
export async function testConnection(event: ToolPkg.AiProviderTestConnectionEvent) {
  await listModels(event as unknown as ToolPkg.AiProviderListModelsEvent);
  return {
    success: true,
    message: "连接成功"
  };
...
export function calculateInputTokens(event: ToolPkg.AiProviderCalculateInputTokensEvent) {
