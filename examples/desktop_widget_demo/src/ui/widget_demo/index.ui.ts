...
function buildTodaySummary() {
  ...
  const greeting =
    hour < 6 ? "凌晨模式" : hour < 12 ? "早上好" : hour < 18 ? "下午好" : "晚上好";
  const focus =
    hour < 12
      ? "优先推进最难的一件事"
      : hour < 18
  ...
  return UI.LazyColumn(
    ...
    [
      ...
      infoCard(
        ctx,
        "这个示例验证什么",
        "route 只负责点击后打开完整页面，render 单独负责桌面小组件的渲染来源。"
      ),
      UI.Button({
        fillMaxWidth: true,
        text: "刷新这次渲染日志",
        onClick: () => {
          console.log(
            `[today_hub] render refreshed: date=${summary.title}, progress=${summary.progress}`
          );
        },
      }),
    ]
