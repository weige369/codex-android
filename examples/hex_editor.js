...
const hexEditorPackage = (function () {
    ...
    function clampPositiveInteger(value, defaultValue, maxValue, fieldName) {
    ...
    function normalizeHexBytesInput(value) {
    ...
    function buildHexDump(bytes, startOffset) {
        if (bytes.length === 0) {
            return "";
        }
        const lines = [];
    ...
    async function ensureFileExists(path, environment) {
    ...
    async function readFileBytes(path, environment) {
    ...
    async function writeFileBytes(path, bytes, environment) {
        return await Tools.Files.writeBinary(path, encodeBytesToBase64(bytes), environment);
    }
    function collectWindow(bytes, offset, length) {
    ...
    async function read_hex_window(params) {
    ...
    async function write_hex_bytes(params) {
        const environment = normalizeEnvironment(params.environment);
        const offset = parseOffset(params.offset);
    ...
    async function find_hex_pattern(params) {
        const environment = normalizeEnvironment(params.environment);
    ...
    async function wrap(fn, params, successMessage, failureMessage) {
        try {
    ...
    return {
        read_hex_window: (params) => wrap(read_hex_window, params, "Hex 窗口读取成功。", "Hex 窗口读取失败"),
        write_hex_bytes: (params) => wrap(write_hex_bytes, params, "Hex 字节写入成功。", "Hex 字节写入失败"),
        find_hex_pattern: (params) => wrap(find_hex_pattern, params, "Hex 模式搜索成功。", "Hex 模式搜索失败")
    };
})();
