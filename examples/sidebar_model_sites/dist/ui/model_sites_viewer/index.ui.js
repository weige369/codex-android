...
function Screen(ctx) {
    ...
    const selectedSiteId = String(ctx.getEnv(site_catalog_js_1.MODEL_SITES_ENV_KEY) || "").trim();
    const selectedSite = (0, site_catalog_js_1.getModelSiteById)(selectedSiteId);
    console.log(`[sidebar_model_sites] viewer render: selectedSiteId=${selectedSiteId || "<empty>"}, found=${selectedSite ? "yes" : "no"}`);
    ...
    const [statusDetail, setStatusDetail] = ctx.useState("statusDetail", "正在准备打开站点");
    ...
    if (!selectedSite) {
        return UI.Box({
            ...
            UI.Button({
                onClick: async () => {
                    await Promise.resolve(ctx.navigate(site_catalog_js_1.MODEL_SITES_LIST_ROUTE));
                },
            }, UI.Text({
                text: "返回列表",
            })),
    ...
    return UI.Box({
        ...
        onLoad: () => {
            console.log(`[sidebar_model_sites] viewer onLoad: site=${selectedSite.name}, url=${selectedSite.url}`);
        ...
        UI.WebView({
            ...
            onPageStarted: () => {
                console.log(`[sidebar_model_sites] webview onPageStarted: site=${selectedSite.name}`);
                setPageLoading(true);
                setPageProgress(12);
            ...
            onProgressChanged: (event) => {
                ...
                console.log(`[sidebar_model_sites] webview onProgressChanged: site=${selectedSite.name}, progress=${nextProgress}`);
                setPageProgress(nextProgress > 0 ? nextProgress : 18);
                setStatusDetail(nextProgress > 0
                    ? `${selectedSite.name} 页面加载中`
                    : `正在建立 ${selectedSite.name} 页面连接`);
            },
            onPageFinished: () => {
                console.log(`[sidebar_model_sites] webview onPageFinished: site=${selectedSite.name}`);
                setPageProgress(100);
                setStatusDetail(`${selectedSite.name} 已打开`);
                setPageLoading(false);
            },
            onReceivedError: (event) => {
                console.log(`[sidebar_model_sites] webview onReceivedError: site=${selectedSite.name}, payload=${JSON.stringify(event ?? {})}`);
            },
        }),
