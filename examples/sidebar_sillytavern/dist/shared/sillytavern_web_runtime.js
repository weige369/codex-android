...
exports.ensureSillyTavernDashboardServer = ensureSillyTavernDashboardServer;
...
const SILLYTAVERN_ARCHIVE_URL = "https://codeload.github.com/SillyTavern/SillyTavern/zip/refs/heads/release";
...
function buildStopExistingServerScript() {
    return [
...
function buildDownloadHelperScript() {
    return [
...
async function prepareRepoFromArchive() {
    ...
    if (!extractedExists?.exists) {
        throw new Error(`SillyTavern extracted directory is missing after zip extraction: ${SILLYTAVERN_EXTRACT_DIR}`);
    }
    await replaceLinuxPath(SILLYTAVERN_EXTRACT_DIR, LINUX_REPO_DIR);
}
async function repoPackageExists() {
    const repoExists = await Tools.Files.exists(`${LINUX_REPO_DIR}/package.json`, "linux");
    return Boolean(repoExists?.exists);
}
async function repoDependenciesInstalled() {
...
async function ensureRepoArchiveDownloaded(onProgress) {
...
async function installRepoDependencies(onProgress) {
    const command = bashCommand([
        buildCommonEnvScript(),
        "if ! command -v pnpm >/dev/null 2>&1; then",
        "  if command -v corepack >/dev/null 2>&1; then",
...
async function startServer(port) {
    const sessionId = await ensureServerTerminalSession();
    await execServerCommand(bashCommand([
        buildCommonEnvScript(),
        `cd ${shellQuote(LINUX_REPO_DIR)}`,
        buildStopExistingServerScript(),
    ].join("\n")), 15000);
    await execServerCommand(`cd ${LINUX_REPO_DIR}`, 4000);
    await execServerCommand(`export HOME=/root && export PNPM_HOME=${LINUX_PNPM_HOME} && export PATH="$PNPM_HOME:$PATH" && export BROWSER=/bin/true`, 4000);
...
async function ensureSillyTavernDashboardServer(params) {
