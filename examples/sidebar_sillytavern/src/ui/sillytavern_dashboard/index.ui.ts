...
import {
  ensureSillyTavernDashboardServer,
  type EnsureSillyTavernDashboardProgressEvent,
  type SillyTavernDashboardServerResult,
} from "../../shared/sillytavern_web_runtime.js";

type EnsureServerResult = SillyTavernDashboardServerResult;
...
export default function Screen(ctx: ComposeDslContext): ComposeNode {
  ...
  const progressBufferRef = ctx.useRef<string>("progressBuffer", "");
  ...
  async function ensureServer(forceRestart: boolean): Promise<void> {
    ...
    try {
      const result = parseToolResult<EnsureServerResult>(
        await ensureSillyTavernDashboardServer({
          force_restart: forceRestart,
          on_progress: (event: EnsureSillyTavernDashboardProgressEvent) => {
        ...
        [
          UI.WebView({
            ...
            onProgressChanged: (event) => {
              ...
              setStatusDetail(
                nextProgress > 0 ? "SillyTavern 页面资源加载中" : "正在建立页面连接"
              );
            },
            onPageFinished: () => {
              setPageProgress(100);
