...
export type GlassSurfaceLiquidProps = {
...
export default function GlassSurfaceLiquid({
  variant,
  className,
  style,
  children,
  ...
  themeMode,
  widthMode
}: GlassSurfaceLiquidProps) {
  const resolvedRadius = radius ?? 20;
  const isDark = themeMode === 'dark';
  const widthClassName = widthMode === 'intrinsic' ? 'is-intrinsic' : 'is-fill';
  const resolvedConfig = useMemo(() => resolveVariantConfig(variant, isDark), [isDark, variant]);
