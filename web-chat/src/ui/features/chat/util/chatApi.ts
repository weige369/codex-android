...
export async function getCodexVersion(token: string): Promise<CodexVersionInfo> {
  return requestJson<CodexVersionInfo>('/api/web/codex/version', token);
}

export async function triggerCodexInstall(
  token: string,
  payload: { version: string; download_url: string; arch: string }
): Promise<{ task_id: string }> {
  return requestJson<{ task_id: string }>(
    '/api/web/codex/install',
    token,
    { method: 'POST' },
    payload
  );
...
export interface CodexInstallStatus {
  task_id: string;
  version: string;
  status: 'pending' | 'downloading' | 'installing' | 'done' | 'error' | 'timeout';
  log: string;
  error?: string | null;
}

export async function getCodexInstallStatus(
  token: string,
  taskId: string
): Promise<CodexInstallStatus> {
  return requestJson<CodexInstallStatus>(
    `/api/web/codex/install/${encodeURIComponent(taskId)}`,
    token
  );
}
